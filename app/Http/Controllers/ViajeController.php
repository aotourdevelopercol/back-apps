    ...
    function listarViajesGenerales(Request $request)
{
    ...
    $codigoEmpleado = DB::table('users as u')
        ->join('empleados_clientes as ec', 'ec.id', '=', 'u.id_empleado_cliente')
        ->where('u.id', '=', $validatedData['app_user_id'] ?? null)
        ->select('ec.codigo_empleado')
        ->first();

    $codigoEmpleadoValor = $codigoEmpleado->codigo_empleado ?? null;

    try {

        $codigoViaje = $validatedData['codigo_viaje'] ?? null;
        ...
        $params = [
            $codigoEmpleadoValor,
            $validatedData['app_user_id'] ?? null,
            $codigoViaje,
            $codigoViaje,
        ];
        ...
        // SOLO si realmente viene el array
        if (!empty($estadoViaje)) {
            $placeholders = implode(',', array_fill(0, count($estadoViaje), '?'));
            $query .= " AND e.codigo IN ($placeholders)";
            $params = array_merge($params, $estadoViaje);
        }
        ...
        if (!empty($estadoViaje) && array_intersect($estadoViaje, ["ENTEND","NOPROMAN","PORAUTORIZAR","PROGRAM"])) {

            $listaVijesPendientes = $this->listarViajesPendientesRutas($codigoEmpleadoValor, $fechaHoy);
            $listaVijesPendientesEjecutivos = $this->listarViajesPendientesEjecutivos($validatedData['app_user_id'] ?? null, $fechaHoy);
        ...
        return Response::json(['response'=>false,'message'=>'Error interno'],500);
    }
}
            ...
            $query = "SELECT
                ...
                LEFT JOIN
                    rutas_solicitadas_pasajeros rsp ON rsp.fk_rutas_solicitadas = rs.id
                        where rsp.empleado_id = ? and rs.montado is null and rs.visible is null and rsp.fk_estado != 80";
    ...
    public function solicitudViajePasajeros(Request $request)
    {
        ...
        END AS accion")
            ...
            ->where('rs.fk_sede', $ciudad_centro_de_costo->fk_sede)
        ...
        if (!$accion) {
            ...
            $rutas_solicitadas = DB::table('rutas_solicitadas as rs')
                ...
                ->where('rs.fk_sede', $ciudad_centro_de_costo->fk_sede)  // Cambiar por la sede del subcentro de costo 
        ...
        try {
            ...
            if (!$rutas_solicitadas || $accion == 1) {
                ...
                $rutas_solicitadas = DB::table('rutas_solicitadas')->insertGetId([
                    ...
                    'fk_sede' => $ciudad_centro_de_costo->fk_sede,  // Cambiar por la sede del subcentro de costo 
